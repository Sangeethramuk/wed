...
export function AssignmentSpecs() {
 ...
 <span className="text-xs font-medium tracking-widest">Back to strategy</span>
 </Button>
 </div>

 <div className="space-y-1">
 <h1 className="text-4xl font-medium tracking-tighter secondary-text">What type of assignment is this?</h1>
 ...
 <div className="space-y-1">
 <h3 className="text-lg tracking-tight font-medium">{t.label}</h3>
 </div>
 </CardContent>
 </Card>
 );
 ...
 <h1 className="text-xl font-medium tracking-tight secondary-text">Assignment Details</h1>
 <Badge variant="outline" className="bg-primary/5 text-primary border-primary/20 font-medium text-xs tracking-widest px-2 h-5 text-center">{assignment.type}</Badge>
 </div>
 <p className="text-muted-foreground text-xs font-semibold opacity-50 tracking-wider">PG/UG Semester VI</p>
 </div>
 </div>
 
 <div className="flex items-center gap-8">
 <div className="text-right space-y-1">
 <p className="text-xs font-medium tracking-widest text-muted-foreground opacity-50">Late Submissions</p>
 ...
 <div className="text-right space-y-1">
 <p className="text-xs font-medium tracking-widest text-muted-foreground opacity-50">Deadline</p>
 <DateTimePicker 
 ...
 <Label className="text-xs font-medium tracking-widest text-muted-foreground opacity-50">Assignment details</Label>
 <Input 
 placeholder="Give your assignment a name — e.g. MVC Architecture Implementation" 
 className="text-lg font-medium h-11 border border-border/60 bg-muted/10 px-4 focus-visible:ring-primary/10 rounded-lg placeholder:opacity-30 tracking-tight"
 ...
 <Label className="text-xs font-medium tracking-widest text-muted-foreground opacity-50">What are students expected to do?</Label>
 <Badge variant="outline" className="text-xs font-medium tracking-widest border-primary/10 text-primary/50 h-4">AI</Badge>
 </div>
 <Textarea 
 ...
 <h3 className="text-xs font-medium tracking-[0.3em] secondary-text flex items-center gap-2 opacity-50">
 ...
 variant="ghost" 
 size="sm"
 className="font-medium text-xs tracking-widest gap-2 opacity-40 hover:opacity-100 h-7 px-3 rounded-md"
 onClick={addSection}
 >
 ...
 <span className="text-xs font-medium text-destructive bg-destructive/5 px-2 py-0.5 rounded border border-destructive/20 whitespace-nowrap">Delete?</span>
 <Button 
 variant="destructive" 
 size="icon" 
 ...
 autoFocus
 value={sec.title}
 className="h-8 text-base font-medium tracking-widest text-primary bg-background border border-primary/20 rounded-md px-3 focus-visible:ring-primary/10"
 ...
 <CardTitle 
 className="text-base font-medium tracking-widest text-primary flex items-center gap-2 cursor-pointer group/title transition-all"
 ...
 <span className="text-xs font-medium tracking-[0.1em] opacity-0 group-hover/title:opacity-100 whitespace-nowrap">Click to rename this section</span>
 </div>
 </CardTitle>
 )}
 <Input 
 value={sec.description}
 className="h-5 border border-border/60 p-2 text-xs text-muted-foreground font-semibold bg-transparent focus-visible:ring-0 opacity-50"
 placeholder="Description..."
 onChange={(e) => updateSection(sec.id, { description: e.target.value })}
 />
 </div>
 <div className="flex flex-col items-end gap-1 opacity-30">
 <Badge variant="outline" className="font-medium text-xs tracking-widest bg-background border-border/40 py-0.5 px-2 rounded-full h-4">v2.1</Badge>
 </div>
 </div>
 </CardHeader>
 ...
 <Label className="text-xs font-medium tracking-widest text-muted-foreground opacity-40">Difficulty level</Label>
 <Tooltip>
 <TooltipTrigger className="text-muted-foreground/30 hover:text-primary transition-colors">
 <HelpCircle className="h-3 w-3" />
 </TooltipTrigger>
 <TooltipContent className="bg-slate-900 border-none p-2 shadow-none rounded-lg"><p className="text-xs font-bold text-white">Helps the AI understand how challenging this task should be.</p></TooltipContent>
 </Tooltip>
 </div>
 <Select value={d.bloomLevel} onValueChange={(val) => { if (val) updateDelivLocal(sec.id, d.id, "bloomLevel", val as BloomLevel) }}>
 <SelectTrigger className="h-9 font-bold text-xs bg-muted/10 border border-border/60 text-primary rounded-md px-3">
 <SelectValue />
 </SelectTrigger>
 ...
 <span>{L.label}</span>
 <span className="text-xs font-medium opacity-50 group-hover:opacity-100">{L.description}</span>
 </div>
 </SelectItem>
 ))}
 </SelectContent>
 ...
 <div className="flex items-center gap-1.5">
 <Label className="text-xs font-medium tracking-widest text-muted-foreground opacity-40">Item name</Label>
 </div>
 <Input 
 value={d.name} 
 onChange={(e) => updateDelivLocal(sec.id, d.id, "name", e.target.value)}
 className="h-9 font-medium text-xs bg-muted/5 border border-border/60 rounded-md px-3 focus-visible:ring-primary/10"
 />
 </div>

 <div className="md:col-span-1 space-y-1.5 text-right">
 <Label className="text-xs font-medium tracking-widest text-muted-foreground opacity-40 block">Marks (%)</Label>
 ...
 className="h-9 w-14 font-medium text-right pr-2 bg-muted/10 border border-border/60 rounded-md focus-visible:ring-primary/10 text-sm"
 />
 <span className="font-medium text-muted-foreground/30 text-xs">%</span>
 </div>
 </div>
 </div>
 ))}
 ...
 <Button 
 variant="secondary" 
 className="w-full border border-border/30 h-10 text-xs font-medium tracking-widest bg-muted/20 hover:bg-primary/5 hover:text-primary hover:border-primary/20 transition-all rounded-lg mt-2 group/add"
 ...
 <CardTitle className="text-xs font-medium tracking-widest text-primary/70 leading-none">
 {hasStarted ? "Completeness" : "Progress"}
 </CardTitle>
 <span className="text-xs font-medium text-muted-foreground opacity-30">
 {hasStarted ? getCompletenessLabel(totalWeight) : "Fill in details to start"}
 </span>
 </div>
 </div>
 <div className="flex flex-col items-end">
 <span className={`text-2xl font-medium tracking-tighter leading-none ${totalWeight === 100 ? 'text-primary' : (hasStarted ? 'text-amber-500' : 'text-muted-foreground/20')}`}>
 {hasStarted ? `${totalWeight}%` : "—"}
 </span>
 <span className="text-xs font-medium opacity-30 tracking-widest">Total</span>
 </div>
 </div>
 </CardHeader>
 ...
 <Tooltip>
 <TooltipTrigger>
 <p className="text-xs font-semibold leading-tight text-foreground/60 cursor-help underline decoration-dotted underline-offset-4">Curriculum alignment: <span className="text-primary font-medium ml-1">{hasStarted ? (90 + Math.floor(totalWeight/10)) : 0}%</span></p>
 </TooltipTrigger>
 <TooltipContent side="left" className="bg-slate-900 border-none p-3 shadow-none rounded-xl">
 <p className="text-xs font-bold text-white">How well this assignment covers your course's learning goals based on what you've filled in so far</p>
 </TooltipContent>
 </Tooltip>
 </div>
 ...
 <p className="text-xs font-semibold leading-tight tracking-widest text-muted-foreground">Learning goals: <span className="text-primary font-medium">{totalWeight > 0 ? "linked" : "pending"}</span></p>
 </div>
 </div>
 
 <div className={cn("pt-6 border-t border-border/10 space-y-5 transition-opacity", !hasStarted && "opacity-20")}>
 <div className="flex items-center justify-between">
 <p className="text-xs font-medium tracking-widest text-muted-foreground opacity-40">Question difficulty</p>
 <Tooltip>
 ...
 <p className="font-medium text-xs tracking-widest text-primary">Difficulty balance</p>
 <p className="text-xs font-medium leading-relaxed text-white">Shows the balance between basic recall questions and higher-order thinking across your assignment.</p>
 </TooltipContent>
 </Tooltip>
 </div>
 <div className="space-y-4">
 <div className="space-y-1.5">
 <div className="flex justify-between text-xs font-medium opacity-50">
