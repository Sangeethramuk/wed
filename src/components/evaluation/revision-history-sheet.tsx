...
function EventCard({ event }: { event: RevisionEvent }) {
 ...
 <span className={`text-xs font-medium tracking-widest ${config.color}`}>
 {config.label}
 </span>
 <Badge
 variant="outline"
...
export function RevisionHistorySheet({
 ...
 <div>
 <SheetTitle className="text-sm font-medium tracking-widest">
 Revision History
 </SheetTitle>
 <SheetDescription className="text-xs tracking-widest font-bold text-muted-foreground/60">
 ...
 key={type}
 variant="outline"
 className={`h-5 px-2 text-xs font-medium tracking-wider rounded-full ${config.bgColor} ${config.color} ${config.borderColor} border`}
 >
 {config.label} ({count})
 </Badge>
 )
 ...
 <div className="flex items-center justify-between text-xs font-medium tracking-widest text-muted-foreground/40">
 <span>
 First event at{" "}
