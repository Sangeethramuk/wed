...
/**
 * Wraps a paragraph that's been flagged for possible plagiarism/cheating.
 * Renders a warning-tinted container with a small pill + hover tooltip
 * ("Review this area carefully"). Inline so multiple flagged paragraphs
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      {elements.map((el, i) => {
        ...
        const wrap = (node: React.ReactNode) =>
          isSuspicious ? <SuspiciousParagraphWrapper key={i}>{node}</SuspiciousParagraphWrapper> : node
        switch (el.type) {
          case "heading":
            return el.level === 3 ? (
