...
function UserHighlightedSpan({ text, criterionId }: { text: string; criterionId: number }) {
 ...
 <span className={`text-xs font-medium tracking-widest ${c.text}`}>Your Evidence</span>
 </div>
...
function HighlightedSpan({
 ...
 <div className={`inline-flex items-center gap-2 px-2 py-1 ${c.badge} rounded-md`}>
 <div className={`w-2 h-2 rounded-full ${c.dot}`} />
 <span className={`text-xs font-medium tracking-widest ${c.text}`}>
 Evidence
 </span>
 </div>
 <div>
 ...
 <span className="text-xs font-medium tracking-widest text-popover-foreground/50">
 AI Confidence
 </span>
...
export default function ManuscriptRenderer({
 ...
 <span className="text-xs font-medium tracking-widest text-slate-400">{el.language}</span>
 ...
 {el.headers.map((h, hi) => (
 <th key={hi} className="px-4 py-2.5 text-xs font-medium tracking-widest text-muted-foreground">{h}</th>
 ))}
 </tr>
 </thead>
 <tbody>
