...
function DeltaBadge({ delta }: { delta: number }) {
  if (delta === 0) return (
    <span className="text-[8px] font-black text-green-600 dark:text-green-400 bg-green-500/10 px-1.5 py-0.5 rounded">Δ 0</span>
  )
  const color = delta === 1
    ? "text-amber-600 dark:text-amber-400 bg-amber-500/10"
    : "text-red-600 dark:text-red-400 bg-red-500/10"
  return (
    <span className={`text-[8px] font-black px-1.5 py-0.5 rounded ${color}`}>Δ {delta}</span>
  )
}
    ...
    <TooltipProvider delay={100}>
      <div className="max-w-5xl mx-auto px-4 space-y-8 animate-in fade-in duration-500 pb-12">
        ...
        <div className="text-center space-y-3 pt-4">
          ...
          <h2 className="text-2xl font-bold tracking-tight">Calibration Comparison</h2>
        ...
        <div className={`flex items-center justify-between p-6 rounded-2xl border-2 ${
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className={`text-[10px] font-black uppercase tracking-[0.2em] ${isCalibrated ? "text-green-600 dark:text-green-400" : "text-amber-600 dark:text-amber-400"}`}>
          ...
          <div className="text-right shrink-0">
            <p className={`text-4xl font-black tracking-tighter tabular-nums ${isCalibrated ? "text-green-600" : "text-amber-600"}`}>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border/30 bg-muted/20">
                  <th className="text-left px-4 py-3 text-[9px] font-black uppercase tracking-[0.2em] text-muted-foreground/50 w-40">Criterion</th>
                  {papers.map(p => (
                    <th key={p.paperId} className="px-3 py-3 text-[9px] font-black uppercase tracking-widest text-muted-foreground/50 text-center min-w-[110px]">
                      {p.anonymizedLabel}
                      <span className={`ml-1.5 text-[7px] px-1 py-0.5 rounded font-black ${
                  ...
                  <th className="px-4 py-3 text-[9px] font-black uppercase tracking-widest text-muted-foreground/50 text-right min-w-[80px]">
                    <Tooltip>
                      <TooltipTrigger>
                  ...
                  return (
                    <tr key={criterion.id} className={`border-b border-border/20 ${idx % 2 === 0 ? "" : "bg-muted/5"}`}>
                      <td className="px-4 py-3">
                        <p className="text-[8px] font-black uppercase tracking-widest text-muted-foreground/40">{criterion.id.toUpperCase()}</p>
              ...
              <tfoot>
                <tr className="border-t border-border/40 bg-muted/10">
                  <td className="px-4 py-2 text-[8px] font-black uppercase tracking-widest text-muted-foreground/30" colSpan={papers.length + 2}>
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <Card key={stat.label} className={`p-5 border ${stat.bg}`}>
              ...
              <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/50 mt-1">{stat.label}</p>
            </Card>
          ))}
        </div>
        ...
        <div className="flex flex-col items-center gap-3 pb-4">
          <Button
            ...
            className={`rounded-full h-14 px-12 text-sm font-black uppercase tracking-widest transition-all hover:scale-[1.02] active:scale-[0.98] group shadow-2xl ${
              isCalibrated
