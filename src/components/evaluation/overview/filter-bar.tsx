...
export function FilterBar() {
 ...
 <span className="text-xs font-medium tracking-widest text-muted-foreground/40 px-1">{group.label}</span>
 {group.options.map(opt => (
 <button
 key={opt.value}
 onClick={() => setFilter(group.key, opt.value)}
 className={`px-2.5 py-1 rounded-md text-xs font-medium tracking-wider transition-all ${
 ...
 size="sm"
 onClick={clearAll}
 className="h-8 px-3 text-xs font-medium tracking-widest text-muted-foreground/60 hover:text-destructive gap-1.5"
 >
