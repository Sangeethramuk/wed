...
function CalibrationBadge({ state }: { state: EvaluationAssignment["calibrationState"] }) {
 ...
 <span className="flex items-center gap-1 text-xs font-medium tracking-widest text-green-600 dark:text-green-400">
 <CheckCircle2 className="h-3 w-3" /> Calibrated
 </span>
 )
 if (state === "in_progress") return (
 <span className="flex items-center gap-1 text-xs font-medium tracking-widest text-amber-600 dark:text-amber-400 animate-pulse">
 <Zap className="h-3 w-3" /> In Progress
 </span>
 )
 return (
 <span className="flex items-center gap-1 text-xs font-medium tracking-widest text-muted-foreground/50">
...
function AssignmentRow({ assignment }: { assignment: EvaluationAssignment }) {
 ...
 <Badge variant="outline" className={`text-xs font-medium tracking-wider shrink-0 ${TYPE_BADGE[assignment.assignmentType] ?? ""}`}>
 {assignment.assignmentType}
 </Badge>
 </div>
 <div className="flex items-center gap-3 text-xs font-medium text-muted-foreground/50 tracking-widest">
 ...
 <p className="text-sm font-medium tabular-nums text-foreground">{assignment.totalSubmissions}</p>
 <p className="text-xs font-medium text-muted-foreground/40 tracking-wider">Papers</p>
 </div>
 ...
 <Tooltip>
 <TooltipTrigger>
 <Badge variant="destructive" className="gap-1 px-2 py-0.5 text-xs font-medium animate-pulse cursor-help">
 ...
 className={`w-full rounded-full h-8 text-xs font-medium tracking-widest transition-all group/btn ${
...
function DeptGroup({ department, assignments }: { department: string; assignments: EvaluationAssignment[] }) {
 ...
 <span className="text-xs font-medium tracking-[0.2em] text-foreground/80">{department}</span>
 <Badge variant="outline" className={`text-xs font-medium tracking-wider border ${DEPT_COLORS[department] ?? ""}`}>
 {assignments.length} assignment{assignments.length !== 1 ? "s" : ""}
 </Badge>
 </div>
 <div className="flex items-center gap-5 text-xs font-medium tracking-widest text-muted-foreground/40">
 <Tooltip>
 ...
 <div className="flex-1 text-xs font-medium tracking-[0.2em] text-muted-foreground/30">Assignment</div>
 <div className="w-16 text-right text-xs font-medium tracking-widest text-muted-foreground/30 shrink-0">Papers</div>
 <div className="w-14 text-center text-xs font-medium tracking-widest text-muted-foreground/30 shrink-0">Flags</div>
 <div className="w-24 text-right text-xs font-medium tracking-widest text-muted-foreground/30 shrink-0">Progress</div>
...
export function AssignmentTable() {
 ...
 <p className="text-sm font-medium text-muted-foreground/40 tracking-widest">No assignments match current filters</p>
 </div>
 )
 }
