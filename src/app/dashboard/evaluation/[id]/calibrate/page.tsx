...
export default function CalibratePage({ params }: { params: Promise<{ id: string }> }) {
 ...
 <p className="text-xs font-medium tracking-widest text-muted-foreground/40">Initializing Calibration Protocol…</p>
 </div>
 </div>
 )
 }
 ...
 <Button variant="ghost" size="sm" className="h-8 px-3 text-xs font-medium tracking-widest text-muted-foreground/50 hover:text-foreground gap-1.5 pl-0">
 <ArrowLeft className="h-3 w-3" /> Triage Desk
 </Button>
 </Link>
 <div className="w-px h-4 bg-border/50" />
 <div className="flex items-center gap-3 text-xs font-medium tracking-[0.2em]">
 ...
 <Badge variant="outline" className="text-xs font-medium tracking-widest bg-muted/30 border-border/30">
 {gradedCount}/{totalPapers} papers graded
 </Badge>
 )}
 <div className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground/25 tracking-widest">
